exports.MostpopularProduct = function(req, res, next) {
  ...
  var profitable = "SELECT sales.product_id, products.product_name, SUM( (sales.quantity * sales_price) - ( cost ) ) profitable FROM sales INNER JOIN purchases ON sales.product_id = purchases.product_id INNER JOIN products ON sales.product_id = products.product_id GROUP BY product_id ORDER BY profitable DESC LIMIT 1";
...
var mostProfitableCategory = " SELECT sales.product_id, products.product_name, categories.category_name, SUM( (sales.quantity * sales_price) - ( cost ) ) mostProfitable FROM sales INNER JOIN purchases ON sales.product_id = purchases.product_id INNER JOIN products ON sales.product_id = products.product_id INNER JOIN categories ON products.category_id = categories.category_id GROUP BY category_name ORDER BY mostProfitable DESC LIMIT 1";
  var leastProfitableCategory = " SELECT sales.product_id, products.product_name, categories.category_name, SUM( (sales.quantity * sales_price) - ( cost ) ) leastProfitable FROM sales INNER JOIN purchases ON sales.product_id = purchases.product_id INNER JOIN products ON sales.product_id = products.product_id INNER JOIN categories ON products.category_id = categories.category_id GROUP BY category_name ORDER BY leastProfitable ASC LIMIT 1";
  ...
  req.getConnection(function(err, connection) {
    connection.query(mostPopularProduct, function(err, max) {
      ...
      connection.query(leastPopularProduct, function(err, min) {
        if (err) return next(err);
        connection.query(profitable, function(err, profitable) {
          if (err) return next(err);
          connection.query(leastProfitable, function(err, leastProfitable) {
            ...
            connection.query(mostPopularCategory, function(err, mostPopularCategory) {
              ...
              connection.query(leastPopularCategory, function(err, leastPopularCategory) {
                if (err) return next(err);
                connection.query(mostProfitableCategory, function(err, mostProfitableCategory) {
                  if (err) return next(err);
                  connection.query(leastProfitableCategory, function(err, leastProfitableCategory) {
                    if (err) return next(err);
                    res.render('statistics', {
                      ...
                      leastProfitableCategory: leastProfitableCategory,
                      leastProfitableCategory: leastProfitableCategory,
